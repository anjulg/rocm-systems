...
// One allocation registered with the playback context.
//
// `size` MUST be the *recorded* (capture-time) original allocation size
// because `translate_ptr` uses it to resolve sub-allocation pointers via
// range search on RECORDED addresses. Storing a padded size here would
// extend the recorded range to cover bytes the capture never used,
// causing range search to falsely match recorded pointers that actually
// belong to a *different* allocation (and silently corrupt kernel input
// pointers — see HRR-padding-range-collision bug).
//
// `padded_size` is the size of the live replay allocation (which may be
// larger than `size` thanks to padding). It is used for live-side bounds
// such as D2H tail-byte clamping (`alloc_bytes_from`).
struct AllocEntry {
    uint64_t rec_base;     // recorded GPU base address
    void*    live_ptr;     // live replay GPU base address
    size_t   size;         // recorded (capture-time) allocation size
    size_t   padded_size;  // live allocation size (>= size; padding for safety)
};
...
struct PlaybackContext {
    ...
    // Exact match first; falls back to sub-alloc range search over the
    // *recorded* original size (NOT the padded live size — using the
    // padded size here causes false positives across unrelated recorded
    // allocations when padding factors are large).
    void* translate_ptr(uint64_t rec) const {
        ...
        // Range search for sub-allocations — bounded by recorded size.
    ...
    // alloc_map entry. Uses the *padded* (live) size since this bounds
    // live-side memory operations such as D2H tail-byte validation.
    // Returns 0 if the pointer is not within any known alloc.
    size_t alloc_bytes_from(void* live_ptr) const {
        ...
        for (auto& [base, entry] : alloc_map) {
            ...
            if (addr >= live_base && addr < live_base + entry.padded_size)
                return entry.padded_size - static_cast<size_t>(addr - live_base);
        }
        return 0;
    }
    ...
    // `sz` is the recorded (capture-time) original size and bounds range
    // search in translate_ptr. `padded_sz` (defaults to `sz`) is the
    // size of the live replay allocation and bounds alloc_bytes_from.
    void record_alloc(uint64_t rec, void* live, size_t sz,
                      size_t padded_sz = 0) {
        if (padded_sz < sz) padded_sz = sz;
        std::unique_lock lk(map_mutex);
        alloc_map[rec] = {rec, live, sz, padded_sz};
    }
