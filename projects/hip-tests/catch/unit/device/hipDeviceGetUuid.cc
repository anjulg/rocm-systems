...
HIP_TEST_CASE(Unit_hipDeviceGetUuid_Positive) {
  ...
  HipTest::printUuid("Uuid: ", 0, uuid);
  REQUIRE(uuidValid == true);
}

/**
 * @addtogroup hipDeviceGetCuid hipDeviceGetCuid
 * @{
 * @ingroup DriverTest
 * `hipDeviceGetCuid(hipUUID* cuid, hipDevice_t device)` -
 * Returns an CUID for the device.
 */

/**
 * Test Description
 * ------------------------
 *  - Check that non-empty CUID is returned for each available device.
 *  - Unlike UUID returned by hipDeviceGetUuid(), CUID is generated by AMD CUID tool or deamon.
 * Test source
 * ------------------------
 *  - unit/device/hipDeviceGetCuid.cc
 * Test requirements
 * ------------------------
 *  - HIP_VERSION >= 7.2
 */
TEST_CASE("Unit_hipDeviceGetCuid_Positive") {
  hipDevice_t device;
  hipUUID cuid{0};
  bool cuidValid = false;

  const int deviceId = GENERATE(range(0, HipTest::getDeviceCount()));
  HIP_CHECK(hipDeviceGet(&device, deviceId));

  // Scenario 1
  HIP_CHECK(hipDeviceGetCuid(&cuid, device));
  // Atleast one non zero value
  size_t cuidSize = sizeof(cuid.bytes) / sizeof(cuid.bytes[0]);
  for (size_t i = 0; i < cuidSize; i++) {
    if (cuid.bytes[i] != 0) {
      cuidValid = true;
      break;
    }
  ...
  if (!cuidValid) {
    fprintf(stderr, "Please start AMD CUID deamon or run the tool,\n" \
		    "  sudo amdcuid_tool --generate-cuid [--generate-key]\n");
    return;
  }
  HipTest::printUuid("Cuid: ", 0, cuid);
  REQUIRE(cuidValid == true);
}
