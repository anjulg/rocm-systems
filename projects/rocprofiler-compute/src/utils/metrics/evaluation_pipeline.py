...
Orchestrates variable creation, expression evaluation, and validation
...
def create_sys_vars(
    ...
    sys_vars_collection: dict[str, int | float] = {}
    ...
    for var_name, var_type, attr_name in sys_vars_config:
        ...
        if np.isnan(variable_value) or variable_value == 0:
            console_warning(
                f"{attr_name} is not available in sysinfo.csv,"
                " please provide the correct value using"
                " --specs-correction"
            )
    ...
    if np.isnan(total_l2_channel_count) or total_l2_channel_count == 0:
        console_warning(
            "total_l2_chan is not available in sysinfo.csv,"
            " please provide the correct value using"
            " --specs-correction"
        )
...
def calc_builtin_vars(
    ...
    builtin_vars_collection: dict[str, Optional[str | float | int]] = {}
            ...
            try:
                ...
                if peak > 0 and value > peak:
                    ...
                    faq_url = (
                        ...
                        "faq.html#why-does-valu-utilization-"
                        "exceed-the-theoretical-peak"
                    )

                    if metric_name in valu_utilization_metrics:
                        warning_msg = (
                            "VALU Utilization can go up to"
                            " 200% because CU can dual-issue"
                            " instructions. See"
                            f" {faq_url} for more information."
                        )
                    else:  # VALU FLOPs metrics
                        warning_msg = (
                            "VALU FLOPs can exceed the peak"
                            " value because these instructions"
                            " can be dual-issued in specific"
                            " circumstances. See"
                            f" {faq_url} for more information."
                        )
