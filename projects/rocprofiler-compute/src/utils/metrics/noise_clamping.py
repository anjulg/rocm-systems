...
Negative counter values are physically impossible -- they arise from
run-to-run variance when hardware counters are collected across
multiple profiling passes.  ``NoiseClamper`` clamps these negatives
to zero and tracks deviations for diagnostics.
"""
...
class NoiseClamper:
    ...
    Negative counts are physically impossible -- they result from
    run-to-run variance when counters are collected across multiple
    profiling passes.  This class clamps negatives to 0 and tracks
    ...
    def print_summary(self) -> None:
        ...
        console_warning(
            f"Counter variance corrected: {self._count} value(s) "
            f"adjusted (max {max_pct:.1f}% deviation from "
            f"multi-pass collection)."
        )
