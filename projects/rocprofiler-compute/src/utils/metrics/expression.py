...
Converts user-defined equation strings (e.g. from YAML metric
definitions) into ``eval()``-ready Python expressions that reference
``raw_pmc_df`` DataFrames.
"""
...
class CodeTransformer(ast.NodeTransformer):
    ...
    def visit_IfExp(self, node: ast.IfExp) -> ast.Expr:
        ...
        if isinstance(node.body, ast.Constant):
            raise Exception(
                "Don't support body of IF with number only!"
                " Has to be expr with df['column']."
            )
...
def build_eval_string(equation: str, coll_level: str, config: dict) -> str:
    ...
    if config.get("format_rocprof_output") == "rocpd":
        ...
        equation_string = re.sub(
            rf"raw_pmc_df['{schema.PMC_PERF_FILE_PREFIX}']"
            r"['SQ_ACCUM_PREV_HIRES']",
    ...
    try:
        tree = ast.parse(
            ...
            .replace(
                ...
                " / $GRBM_GUI_ACTIVE_PER_XCD})), 0) /"
                " $maxWavesPerCU) * 8) + MIN(MOD(ROUND(AVG((("
                "4 * SQ_BUSY_CU_CYCLES) /"
                " $GRBM_GUI_ACTIVE_PER_XCD)), 0),"
                " $maxWavesPerCU), 8)), $numCU))",
            )
            .replace("$", "")
        )
