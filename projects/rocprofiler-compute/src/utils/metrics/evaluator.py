...
"""MetricEvaluator -- safe ``eval()`` wrapper for YAML metric expressions."""
...
class MetricEvaluator:
    ...
    def eval_expression(self, expr: str) -> str | float | int:
        ...
        try:
            ...
            local_expr_context["raw_pmc_df"] = self.raw_pmc_df
            ...
            local_expr_context.update(_AGGREGATION_FUNCTIONS)
            ...
            if (
                ...
                if "None" not in expr:
                    console_warning(
                        ...
                        f" '{expr}' - likely due to missing"
                        " counter data."
                    )
                else:
                    console_debug(
                        f"Expression '{expr}' evaluated to"
                        " None - likely explicitly specified."
                    )
                return "N/A"
            else:
                return eval_result
