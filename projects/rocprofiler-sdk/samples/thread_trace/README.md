...
- Receives the trace data in `shader_data_callback` and calls `rocprof_trace_decoder_parse_data` to decode the data
- `rocprof_trace_decoder_parse_data` calls `parse` (a lambda)
