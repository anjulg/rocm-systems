"""Interactive session for perfxpert analyze --interactive."""
...
_DEFAULT_SESSIONS_DIR = pathlib.Path.home() / ".perfxpert" / "sessions"
...
def _print_startup_banner() -> None:
    ...
    if art:
        ...
        if _RICH and _console:
            _console.print(
                f"  [bold white]PerfXpert[/bold white]"
                f"  [dim]·[/dim]"
            ...
            print(
                f"  {_A_BOLD}{_A_WHITE}PerfXpert{_A_RESET}"
                f"  {_A_DIM}·{_A_RESET}"
...
class InteractiveSession:
    """Top-level interactive menu for perfxpert analyze --interactive."""
        ...
        if _RICH and _console:
            _console.print(
                Panel(
                    ...
                    title=f"[bold {_AMD_RED}]▌[/bold {_AMD_RED}] [bold white]PerfXpert[/bold white]  [dim]AMD ROCm AI Analysis[/dim]",
        ...
        if llm_provider == "local" and not self._llm_local:
            ...
            if not detected:
                ...
                if fallback:
                    ...
                    _print(
                        "    • Or pass --llm anthropic / --llm openai when launching perfxpert.",
                        style="dim",
                    )
                    return
    ...
    def _autodetect_llm(self) -> Optional[str]:
        ...
        try:
            ...
            url = os.environ.get("PERFXPERT_LLM_LOCAL_URL", "http://localhost:11434")
...
class CheckpointRecord:
    ...
    worktree_path: str  # ~/.perfxpert/sessions/<id>/cp-N
...
class GitCheckpointManager:
    ...
    _ID = ["-c", "user.email=perfxpert@local", "-c", "user.name=perfxpert"]
    ...
    def create_checkpoint_commit(self, files: List[str], message: str) -> str:
        ...
        Uses --no-verify to skip pre-commit hooks (perfxpert checkpoints are
    ...
    def tag_checkpoint(self, cp_id: int, commit_hash: str) -> str:
        ...
        Returns the ref name: refs/perfxpert/<session_id>/cp-N
        """
        ref_name = f"refs/perfxpert/{self._session_id}/cp-{cp_id}"
...
class WorkflowSession:
    ...
    Triggered by: perfxpert analyze --interactive "<app_command>"
    """

    _DEFAULT_TRACE_DIR = "/tmp/perfxpert_trace"
    ...
    def __init__(
        ...
        self._sessions_dir = pathlib.Path.home() / ".perfxpert" / "sessions"
    ...
    def _load_session(
        ...
        Accepts:
          ...
          - A session ID (looked up in ~/.perfxpert/sessions/<id>.json)
    ...
    def _save_session(self) -> None:
        """Serialize WorkflowState to ~/.perfxpert/sessions/workflow_<id>.json."""
    ...
    def _create_checkpoint(
        ...
        message = f"perfxpert: cp-{cp_id} — {edit_summary}"
    ...
    def _teardown_checkpoints(self) -> None:
        ...
        Refs (refs/perfxpert/…) are kept so commits survive GC until
        the user runs 'perfxpert sessions --cleanup'.
        """
        if self._gcm is None:
            return
        for cp in self._state.checkpoints:
    ...
    def _prune_stale_worktrees(self) -> None:
        """Remove worktrees under ~/.perfxpert/sessions/ with no matching session JSON.
    ...
    def _post_revert_llm_analysis(
        ...
        if analysis is None:
            ...
            try:
                from perfxpert.ai_analysis.llm_analyzer import LLMAnalyzer  # type: ignore[import]
    ...
    def _merge_per_process_dbs(self, db_files: List[str], out_dir: str) -> Optional[str]:
        ...
        try:
            ...
            pass
        try:
            from perfxpert.connection import merge_sqlite_dbs  # type: ignore[import]
    ...
    def _phase4_analyze(self, db_path: str) -> _AnalysisSnapshot:
        ...
        try:
            from perfxpert.ai_analysis.api import analyze_database  # type: ignore[import]
    ...
    def print_session_summary(self) -> None:
        ...
        if self._session_file.exists():
            ...
            _print(
                f"  Resume     : perfxpert analyze --interactive "
                f'"{self._state.app_command}" '
