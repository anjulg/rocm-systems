...
Run with system perfxpert first in PYTHONPATH:

    PERFXPERT_SYS=$(python3 -c "import site; print(site.getsitepackages()[-1])")
    PERFXPERT_SRC=<repo>/projects/rocprofiler-sdk/source/lib/python
    PYTHONPATH="${PERFXPERT_SYS}:${PERFXPERT_SRC}" pytest --noconftest test_workflow.py -v
"""
...
# If PERFXPERT_SYS is set, ensure the system-installed rocpd wins over any path that
# pytest may have prepended during package-discovery (e.g. the build tree).
_PERFXPERT_SYS = os.environ.get("PERFXPERT_SYS", "")
if _PERFXPERT_SYS:
    if not os.path.isdir(_PERFXPERT_SYS):
        pytest.skip(
            f"PERFXPERT_SYS={_PERFXPERT_SYS!r} does not exist; skipping workflow tests",
            allow_module_level=True,
        )
    sys.path.insert(0, _PERFXPERT_SYS)
    # Purge any partially-initialised perfxpert loaded from the wrong tree.
    for _key in list(sys.modules):
        if _key == "perfxpert" or _key.startswith("perfxpert."):
            del sys.modules[_key]

from perfxpert.ai_analysis.interactive import WorkflowState  # noqa: E402
...
def test_checkpoint_record_defaults():
    from perfxpert.ai_analysis.interactive import CheckpointRecord

    cp = CheckpointRecord(
        cp_id=0,
        commit_hash="abc1234",
        ref_name="refs/perfxpert/session-1/cp-0",
...
def test_checkpoint_error_is_exception():
    from perfxpert.ai_analysis.interactive import CheckpointError
...
def test_workflow_state_has_checkpoint_fields():
    from perfxpert.ai_analysis.interactive import WorkflowState
...
def test_edit_record_has_checkpoint_id():
    from perfxpert.ai_analysis.interactive import _EditRecord
...
def _make_gcm(repo_root="/repo", session_id="sess1"):
    from perfxpert.ai_analysis.interactive import GitCheckpointManager

    return GitCheckpointManager(
        repo_root=repo_root,
        session_id=session_id,
        sessions_dir="/home/user/.perfxpert/sessions",
    )
...
def test_gcm_create_checkpoint_commit_passes_identity():
    ...
    for c in mock_run.call_args_list:
        assert "perfxpert@local" in str(c)
...
def test_gcm_tag_checkpoint():
    ...
    assert ref == "refs/perfxpert/sess1/cp-0"
...
def test_gcm_tag_checkpoint_not_a_branch():
    ...
    assert ref.startswith("refs/perfxpert/")
...
def test_gcm_add_worktree():
    ...
    assert path == "/home/user/.perfxpert/sessions/sess1/cp-0"
...
def test_gcm_delete_ref():
    ...
    with patch("subprocess.run") as mock_run:
        mock_run.return_value = MagicMock(returncode=0)
        gcm.delete_ref("refs/perfxpert/sess1/cp-0")
...
def test_gcm_list_worktrees():
    ...
    with patch("subprocess.run") as mock_run:
        mock_run.return_value = MagicMock(
            returncode=0,
            stdout="worktree /repo\nHEAD abc\n\nworktree /home/user/.perfxpert/sessions/s/cp-0\nHEAD def\n",
        )
        paths = gcm.list_worktrees()
    assert "/repo" in paths
    assert "/home/user/.perfxpert/sessions/s/cp-0" in paths
...
def test_init_checkpoints_disables_on_get_head_failure():
    from perfxpert.ai_analysis.interactive import WorkflowSession, CheckpointError
...
def _make_workflow_session_with_gcm(mock_gcm=None):
    ...
    from perfxpert.ai_analysis.interactive import WorkflowSession, GitCheckpointManager
...
def test_create_checkpoint_appends_checkpoint_record():
    ...
    ws._gcm.tag_checkpoint.return_value = "refs/perfxpert/sess/cp-0"
...
def test_create_checkpoint_links_edit_record():
    ...
    ws._gcm.tag_checkpoint.return_value = "refs/perfxpert/s/cp-0"
    ws._gcm.add_worktree.return_value = "/tmp/cp-0"
    from perfxpert.ai_analysis.interactive import _EditRecord
...
def test_update_checkpoint_records_run_index():
    from perfxpert.ai_analysis.interactive import (
        CheckpointRecord,
        _AnalysisSnapshot,
        _TraceRun,
    )
...
def _make_ws_with_checkpoints():
    ...
    for i, delta in enumerate(deltas):
        cp = CheckpointRecord(
            ...
            ref_name=f"refs/perfxpert/s/cp-{i}",
...
def test_phase5_shows_rollback_option_when_checkpoints_exist():
    ws = _make_ws_with_checkpoints()
    from perfxpert.ai_analysis.interactive import _AnalysisSnapshot
...
def test_phase5_does_not_crash_when_no_checkpoints():
    from perfxpert.ai_analysis.interactive import WorkflowSession, _AnalysisSnapshot
...
def test_priority_badge_renders_visible_brackets():
    ...
    from perfxpert.ai_analysis.interactive import _priority_badge
