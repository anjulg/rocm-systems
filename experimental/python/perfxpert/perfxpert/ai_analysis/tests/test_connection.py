#!/usr/bin/env python3
"""
Tests for perfxpert.connection — PerfxpertConnection and merge_sqlite_dbs.
...
from perfxpert.connection import PerfxpertConnection, execute_statement, merge_sqlite_dbs
...
@pytest.fixture()
def two_dbs(tmp_dir):
    """Two databases both containing pmc_events (which IS in _PERFXPERT_TABLES)."""
...
class TestSingleFileConnection:
    def test_opens_single_db_directly(self, single_db):
        conn = PerfxpertConnection(single_db)
    ...
    def test_accepts_string_path(self, single_db):
        conn = PerfxpertConnection(str(single_db))
    ...
    def test_context_manager(self, single_db):
        with PerfxpertConnection(single_db) as conn:
            rows = conn.execute("SELECT name FROM kernels").fetchall()
        assert len(rows) == 1
...
class TestMultiFileConnection:
    def test_multi_file_creates_in_memory_union(self, two_dbs):
        ...
        p1, p2 = two_dbs
        conn = PerfxpertConnection([p1, p2])
        # pmc_events is in _PERFXPERT_TABLES so a UNION ALL view is created for it
        rows = conn.execute(
...
class TestPartialSchema:
    def test_table_missing_from_one_shard_skipped_gracefully(self, tmp_dir):
        ...
        conn = PerfxpertConnection([db0, db1])
        conn.close()
...
class TestExecuteStatement:
    def test_execute_statement_on_perfxpert_connection(self, single_db):
        conn = PerfxpertConnection(single_db)
