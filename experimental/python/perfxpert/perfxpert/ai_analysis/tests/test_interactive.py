...
class TestInteractiveSessionMenu:
    def test_new_session_created_when_none_exist(self, tmp_path):
        store = SessionStore(sessions_dir=tmp_path)
        with patch("perfxpert.ai_analysis.interactive._input", return_value=""):
    ...
    def test_quit_saves_session(self, tmp_path):
        store = SessionStore(sessions_dir=tmp_path)
        with patch("perfxpert.ai_analysis.interactive._input", side_effect=["q"]):
            s = InteractiveSession(
                source_dir="/tmp/myapp",
                tier0_result=None,
                recommendations=[],
    ...
    def test_run_save_without_quit(self, tmp_path):
        ...
        store = SessionStore(sessions_dir=tmp_path)
        with patch("perfxpert.ai_analysis.interactive._input", side_effect=["s", "q"]):
            s = InteractiveSession(
                source_dir="/tmp/myapp",
                tier0_result=None,
                recommendations=[],
    ...
    def test_run_eof_saves_and_exits(self, tmp_path):
        ...
        store = SessionStore(sessions_dir=tmp_path)
        with patch("perfxpert.ai_analysis.interactive._input", side_effect=EOFError()):
            s = InteractiveSession(
                source_dir="/tmp/myapp",
                tier0_result=None,
                recommendations=[],
    ...
    def test_run_numeric_pursues_recommendation(self, tmp_path):
        ...
        with patch("perfxpert.ai_analysis.interactive._input", side_effect=["1", "q"]):
            s = InteractiveSession(
                source_dir="/tmp/myapp",
                tier0_result=None,
                recommendations=[],
    ...
    def test_prompt_resume_invalid_choice_starts_new(self, tmp_path):
        ...
        with patch("perfxpert.ai_analysis.interactive._input", return_value="99"):
            s = InteractiveSession(
                source_dir="/tmp/myapp",
                tier0_result=None,
                recommendations=[],
...
class TestPathProfiling:
    ...
    def test_path_p_adds_history_entry_on_db_provided(self, tmp_path):
        ...
        with patch("perfxpert.ai_analysis.interactive._input", side_effect=input_seq):
...
class TestPursueRecommendation:
    def test_pursue_back_to_menu_keeps_item(self, tmp_path):
        ...
        s.session.persistent_menu_items.append(item)
        with patch("perfxpert.ai_analysis.interactive._input", return_value="m"):
