#!/usr/bin/env python3
"""
Tests for core analysis functions in perfxpert.analyze.

Uses in-memory SQLite databases via PerfxpertConnection for isolation.
...
def _make_connection(setup_sql: str) -> PerfxpertConnection:
    """
    Create a temporary on-disk SQLite database, run setup_sql, and return
    a PerfxpertConnection pointing at it.
    We cannot use ":memory:" directly because PerfxpertConnection wraps a
...
class _TmpConnection:
    """Wraps PerfxpertConnection so the temp file is cleaned up after use."""
    def __init__(self, path: str):
        self._path = path
        self._conn = PerfxpertConnection(path)
