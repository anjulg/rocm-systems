...
Run:
    pytest perfxpert/ai_analysis/tests/test_tracelens_port.py -v

Integration test — auto-generates a trace DB with rocprofv3 when available.
Falls back to PERFXPERT_TEST_DB env var if rocprofv3 is not installed.
"""
...
class TestMergeIntervals:
    def test_empty(self):
        from perfxpert.tracelens_port import _merge_intervals
...
class TestCategorizeKernelName:
    def test_gemm(self):
        from perfxpert.tracelens_port import categorize_kernel_name
...
class TestSubtractIntervals:
    def test_empty_a(self):
        from perfxpert.tracelens_port import _subtract_intervals
...
class TestComputeIntervalTimeline:
    def test_overlapping_kernels(self):
        ...
        from perfxpert.tracelens_port import compute_interval_timeline
        ...
        memcpy_rows = []
        with patch("perfxpert.tracelens_port.execute_statement") as mock_es:
            mock_es.side_effect = [
...
class TestAnalyzeKernelsByCategory:
    def test_basic(self):
        ...
        from perfxpert.tracelens_port import analyze_kernels_by_category
...
class TestAnalyzeShortKernels:
    def test_basic(self):
        ...
        from perfxpert.tracelens_port import analyze_short_kernels
...
_SKIP_REASON = (
    "rocprofv3 not found and PERFXPERT_TEST_DB not set — "
    "install ROCm or export PERFXPERT_TEST_DB=/path/to/trace.db"
)
...
def _find_repo_root() -> Path:
    ...
    # Fallback: return the package root (experimental/python/perfxpert/) as a safe default.
...
def _find_app(repo_root: Path) -> str:
    ...
    candidates = [
        ...
        os.environ.get("PERFXPERT_DEMO_APP", ""),
...
def integration_db(tmp_path_factory):
    ...
    Priority: auto-generate via rocprofv3 > PERFXPERT_TEST_DB env var.
    The temporary directory is managed by pytest and cleaned up after the session.
    """
    fallback = os.environ.get("PERFXPERT_TEST_DB", "")
    ...
    out_dir = str(tmp_path_factory.mktemp("perfxpert_integ"))
...
def test_integration_tracelens_with_real_db(integration_db):
    ...
    conn = PerfxpertConnection([integration_db])
