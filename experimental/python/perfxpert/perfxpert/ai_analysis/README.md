...
# Local Ollama model
perfxpert analyze -i output.db --llm-local ollama --llm-local-model llama3

# With custom analysis prompt
perfxpert analyze -i output.db --llm anthropic --prompt "Why is my matmul kernel slow?"

# JSON output (produces analysis.json)
perfxpert analyze -i output.db --format json -d ./output -o analysis

# Markdown output (produces analysis.md)
perfxpert analyze -i output.db --format markdown -d ./output -o analysis
...
perfxpert analyze --source-dir ./my_app --format json -d ./output -o plan

# Combined: Tier 0 + Tier 1/2
perfxpert analyze -i output.db --source-dir ./my_app
...
perfxpert analyze --interactive "./my_app arg1" --llm anthropic
...
- In interactive sessions, perfxpert **auto-detects** ATT by checking the `.db` parent
...
| `--llm private` | `PERFXPERT_LLM_PRIVATE_URL` | `PERFXPERT_LLM_PRIVATE_MODEL` | Any OpenAI-compatible endpoint |
| `--llm local` | `PERFXPERT_LLM_LOCAL_URL` | `PERFXPERT_LLM_LOCAL_MODEL` | Local Ollama |

Override model: `PERFXPERT_LLM_MODEL` env var or `--llm-model <model-name>`.
...
Invoked with `perfxpert analyze -i output.db --interactive` (no app string):
...
Invoked with `perfxpert analyze --interactive "./my_app arg1"`:
...
Sessions save to `~/.perfxpert/sessions/`.  `--resume-session` applies to `InteractiveSession` only.
...
```bash
cd projects/perfxpert

# Unit tests (no .db or GPU required)
PYTHONPATH=. python3 -m pytest perfxpert/ai_analysis/tests --noconftest -v
...
| `PERFXPERT_LLM_MODEL` | Override default LLM model |
| `PERFXPERT_LLM_REFERENCE_GUIDE` | Path to custom reference guide |
| `PERFXPERT_LLM_PRIVATE_URL` | Private server base URL (required for `--llm private`) |
| `PERFXPERT_LLM_PRIVATE_MODEL` | Private server model name |
| `PERFXPERT_LLM_PRIVATE_API_KEY` | Private server API key (default: `"dummy"`) |
| `PERFXPERT_LLM_PRIVATE_HEADERS` | Extra HTTP headers as JSON / Python-dict |
| `PERFXPERT_LLM_PRIVATE_VERIFY_SSL` | `0` = disable SSL verification (requires `httpx`) |
| `PERFXPERT_LLM_LOCAL_URL` | Ollama base URL (default: `http://localhost:11434/v1`) |
| `PERFXPERT_LLM_LOCAL_MODEL` | Ollama model (default: `codellama:13b`) |
