# PerfXpert AI Analysis Output - JSON Schema Changelog

This document tracks all changes to the JSON output schema for `perfxpert analyze --format json`
and the `perfxpert.ai_analysis` Python API.
...
schema_path = pkg_resources.files("perfxpert.ai_analysis") / "docs" / "analysis-output.schema.json"
```
...
- Webview `<title>`, HTML logo span, and markdown output headers changed from
  "ROCpd AI Performance Analysis" to "PerfXpert AI Performance Analysis".
- `analysis-output.schema.json` `title` field updated to "PerfXpert AI Analysis Output".

**Output routing fix (`-d` without `-o`):**
- When `-d <dir>` is given without `-o <name>`, perfxpert now auto-generates the output
...
- When `_call_openai` receives empty content with `finish_reason=="length"` (reasoning
  ...
  `<!-- perfxpert-context: always -->` sections and `max_completion_tokens=16384`.
...
**`PERFXPERT_LLM_PRIVATE_HEADERS` dict validation:**
...
New fields and values added to support `perfxpert analyze --source-dir` (no database required):
...
**No schema changes.** `perfxpert.ai_analysis` Python API bug fixes and hardening (audit AIA-001 through AIA-013).
...
- AIA-009: Added `DEFAULT_ANTHROPIC_MODEL` / `DEFAULT_OPENAI_MODEL` constants. Model names
  are now configurable via `PERFXPERT_LLM_MODEL` environment variable at runtime or the new
  `--llm-model` CLI flag (`perfxpert analyze --llm anthropic --llm-model claude-opus-4-6`).
...
- AIA-011: `ReferenceGuideNotFoundError` is now exported from `perfxpert.ai_analysis.__init__`.
...
- Redesigned webview layout inspired by AMD dashboard design language:
  - **Light/Dark theme toggle** — persisted in `localStorage` (`perfxpert-theme` key);
...
schema_text = (
    pkg_resources.files("perfxpert.ai_analysis")
