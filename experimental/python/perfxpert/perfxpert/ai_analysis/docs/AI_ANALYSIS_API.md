# perfxpert AI Analysis Python API Documentation

**Version:** 0.4.0
**Module:** `perfxpert.ai_analysis`

> **Note:** This module was extracted from `rocpd` (rocprofiler-sdk) into the standalone
> `perfxpert` package.  All import paths use `perfxpert.ai_analysis` instead of
> `perfxpert.ai_analysis`.  The API, data structures, and behavior are identical.
...
The perfxpert AI Analysis API provides programmatic access to AI-powered GPU performance analysis. It's designed for integration with visualization tools (like Optiq), automated analysis pipelines, and custom workflows.
...
perfxpert is a standalone package — no rocprofiler-sdk build required.

```bash
# perfxpert can be installed standalone:
pip install ./projects/perfxpert
...
```python
from perfxpert.ai_analysis import analyze_database_to_json
from pathlib import Path
...
```bash
perfxpert analyze -i output.db --format webview -d ./output -o analysis
...
```python
from perfxpert.ai_analysis import analyze_database, OutputFormat
from pathlib import Path
...
```bash
perfxpert analyze --source-dir ./my_app/src
perfxpert analyze --source-dir ./my_app/src --format json -d ./out -o plan  # → plan.json

# Combined with trace database
perfxpert analyze -i output.db --source-dir ./my_app/src
```
...
- **Private/enterprise server** (any OpenAI-compatible endpoint)
  ...
  - Required env var: `PERFXPERT_LLM_PRIVATE_URL` — base URL (e.g. `https://llm-api.example.com/OpenAI`)
  - Required: `PERFXPERT_LLM_PRIVATE_MODEL` or `--llm-private-model`
  - Optional: `PERFXPERT_LLM_PRIVATE_API_KEY` (default: `"dummy"` for header-authenticated servers)
  - Optional: `PERFXPERT_LLM_PRIVATE_HEADERS` — JSON object of extra request headers;
  ...
  - Optional: `PERFXPERT_LLM_PRIVATE_VERIFY_SSL=0` — disable SSL certificate verification (requires `httpx`)
...
- **Local Ollama**
  ...
  - Env var: `PERFXPERT_LLM_LOCAL_URL` (default: `http://localhost:11434/v1`)
  - Env var: `PERFXPERT_LLM_LOCAL_MODEL` (default: `codellama:13b`)

**Override the model at runtime** (anthropic/openai providers):

```bash
export PERFXPERT_LLM_MODEL="claude-opus-4-6"   # Use a different Anthropic model
export PERFXPERT_LLM_MODEL="gpt-4o"            # Use a different OpenAI model
```
...
```python
from perfxpert.ai_analysis import analyze_database, get_recommendations
from pathlib import Path
import sys
...
- [CLI Documentation](../README.md) - Using `perfxpert analyze` command
...
```python
from perfxpert.ai_analysis import LLMConversation
...
from perfxpert.ai_analysis import load_reference_guide
...
| `api_key` | `None` | API key; falls back to `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` / `PERFXPERT_LLM_PRIVATE_API_KEY` |
| `model` | `None` | Model override; falls back to `PERFXPERT_LLM_MODEL` then built-in default |
...
1. `PERFXPERT_LLM_REFERENCE_GUIDE` environment variable path
...
Previously, the `model` parameter was stored but the `PERFXPERT_LLM_MODEL` environment
...
2. `PERFXPERT_LLM_MODEL` environment variable
...
1. System prompt filtered to only `<!-- perfxpert-context: always -->` sections
...
perfxpert analyze -i merged_opt.db --format webview -d ./reports
```
...
`perfxpert analyze -i db.db --interactive` **without** a `"<app_command>"` string).

`WorkflowSession` (7-phase workflow, triggered by `perfxpert analyze --interactive "<app>"`)
...
2. `_init_session(resume_id)` loads the session JSON from `~/.perfxpert/sessions/`.
...
```bash
ls ~/.perfxpert/sessions/*.json | xargs -I{} python3 -c \
...
Phase 6 AI edit
  ...
  └─► git update-ref refs/perfxpert/<session_id>/cp-N  (GC-pinned ref, not a branch)
  └─► git worktree add --detach ~/.perfxpert/sessions/<session_id>/cp-N
...
worktrees. Refs (`refs/perfxpert/…`) are kept so the commits survive GC until the user
...
| `ref_name` | `str` | `refs/perfxpert/<session_id>/cp-<N>` |
...
gcm.tag_checkpoint(commit_hash=hash_, cp_id=0)          # creates refs/perfxpert/.../cp-0
...
`commit_files` uses `-c user.email=perfxpert@local -c user.name=rocpd` overrides and
