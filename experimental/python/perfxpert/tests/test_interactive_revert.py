...
def _mock_llm(analysis_text: str = _ANALYSIS_TEXT):
    ...
    with (
        ...
        patch("perfxpert.ai_analysis.llm_analyzer.LLMAnalyzer", autospec=True) as M,
        patch(
            "perfxpert.ai_analysis.interactive._Spinner",
...
def _run(
    ...
    with (
        ...
        patch("perfxpert.ai_analysis.interactive._input", side_effect=lambda p="": next(inputs)),
...
class TestAlternativeApplyCompileWait:
    ...
    def test_no_llm_provider_skips_analysis(self, tmp_path):
        ...
        inputs = iter(["p"])
        with patch("perfxpert.ai_analysis.interactive._input", side_effect=lambda p="": next(inputs)):
            action = sess._post_revert_llm_analysis(
                file_path=src,
                original_content=_ORIGINAL,
                failed_content=_FAILED,
...
class TestWorkflowConversation:
    ...
    def test_ensure_conv_creates_conversation(self, tmp_path):
        ...
        with patch(
            "perfxpert.ai_analysis.interactive.LLMConversation"
        ) as MockConv:
...
class TestCommandValidation:
    ...
    def test_safe_command_returns_false(self):
        ...
        from perfxpert.ai_analysis.interactive import _has_shell_meta
